describe("Cypress + TypeScript setup", () => {
  it("loads the example page and checks content", () => {
    ...
    cy.wait(5000);
    //cy.get(`canvas`).eq(2).should("exist").click().screenshot('Histogram_Test');
    // cy.get(`#unity-canvas`).should("exist").wait(10000).screenshot("test");

    cy.fixture("Images/Histogram_Test.png", "base64").then((baselineBase64) => {
      cy.get(`canvas`)
        .eq(2)
        .then(($canvas) => {
          ...
          cy.window().then((win) => {
            ...
            return new Promise<void>((resolve, reject) => {
              baselineImg.onload = () => {
                // Ensure same size
                if (canvasEl.width !== baselineImg.width || canvasEl.height !== baselineImg.height) {
                  return reject(
